...
namespace Schiism.Core.Models.IPC
{
    ...
    public class PipeSerializer
    {
            ...
            CancellationToken ct = default)
        {
            ...
            byte[] payload = JsonSerializer.SerializeToUtf8Bytes(value, this.jsonOptions);
        ...
        public async Task<T?> DeserializeAsync<T>(
            ...
            CancellationToken ct = default)
        {
            byte[] lengthPrefix = await this.ReadExactAsync(stream, 4, ct);
            ...
            byte[] payload = await this.ReadExactAsync(stream, length, ct);

            return JsonSerializer.Deserialize<T>(payload, this.jsonOptions);
        }
